...
const MessageContainer = ({ onBackUser }) => {
    ...
    return (
        <div className='md:min-w-[500px] h-[99%] flex flex-col py-2 bg-gray-900'>
            {selectedConversation === null ? (
                ...
                <>
                    <div className='flex justify-between gap-1 bg-sky-700 md:px-2 rounded-lg h-10 md:h-12'>
                        <div className='flex gap-2 md:justify-between items-center w-full'>
                            <div className='md:hidden ml-1 self-center'>
                                <button onClick={() => onBackUser(true)} className='bg-gray-800 rounded-full px-2 py-1 self-center'>
                                    <IoArrowBackSharp size={25} className='text-white' />
                                </button>
                            </div>
                            <div className='flex justify-between mr-2 gap-2'>
                                <div className='self-center'>
                                    <img className='rounded-full w-6 h-6 md:w-10 md:h-10 cursor-pointer' src={selectedConversation?.profilepic} alt="Profile" />
                                </div>
                    ...
                    <div className='flex-1 overflow-auto bg-gray-800'>
                        ...
                        {!loading && messages?.length === 0 && (
                            <p className='text-center text-white items-center'>Send a message to start the conversation</p>
                        )}
                        {!loading && messages?.length > 0 && messages?.map((message) => (
                            <div className='text-white' key={message?._id} ref={lastMessageRef}>
                                <div className={`chat ${message.senderId === authUser._id ? 'chat-end' : 'chat-start'}`}>
                                    <div className='chat-image avatar'></div>
                                    <div className={`chat-bubble ${message.senderId === authUser._id ? 'bg-sky-600' : 'bg-gray-700'}`}>
                                        {message?.message}
                                    </div>
                    ...
                    <form onSubmit={handelSubmit} className='rounded-full text-black'>
                        <div className='w-full rounded-full flex items-center bg-gray-700'>
                            <input value={sendData} onChange={handelMessages} required id='message' type='text' 
                            className='w-full bg-transparent outline-none px-4 text-white rounded-full' />
                            <button type='submit'>
